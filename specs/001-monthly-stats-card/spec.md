...
- The card targets HA version 2026.5.0 and later; compatibility with older versions is not guaranteed. The HA statistics WebSocket API replaces the deprecated `has_mean` flag with an explicit `mean_type` parameter from HA 2026.11; the implementation must pass `mean_type` explicitly to remain compatible with HA 2026.11+.
