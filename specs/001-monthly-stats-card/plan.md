...
- [x] **III. Density & Data Fidelity** — CSS Grid with 4px cell padding, zero decorative gap. Computation rules: `measurement` → min/avg/max from HA daily stats; monthly min/mean/max card-recomputed from daily `DailyValue` entries (HA monthly `min`/`max` are period-mean extremes, not true daily extremes — see T049). Cumulative → daily delta (`sum[N] − sum[N-1]`); negative as 0 for `total_increasing`, as-is for `total`. Precipitation only (`device_class: precipitation`): zero-sum days excluded from monthly summary (FR-016); all other cumulative entities include zero-sum days.
