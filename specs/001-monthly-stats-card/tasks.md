...
- [ ] T039 [US5] Verify `de-AT` `Intl` behaviour in `frontend/tests/unit/localize/localize.test.ts`: confirm `new Intl.DateTimeFormat('de-AT', {month:'long'}).format(new Date(2026,0))` returns `'Jänner'` under happy-dom; confirm all 12 Austrian month names are correct; confirm `Intl.NumberFormat('de-AT').format(1234.5)` uses comma decimal separator
...
- [ ] T041 [P] Run full test suite in WSL (`npm test`) and confirm all tests pass; run `npm run test:coverage`; verify branch coverage ≥ 80% for `data-transform.ts` and `statistics-service.ts`; if below 80%, write additional unit tests targeting uncovered branches before proceeding to T042
