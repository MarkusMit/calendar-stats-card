...
Below the summary table, the comparison view shows the selected month's daily values for each compared year, presented like the familiar monthly view.
...
1. **Given** the comparison view is open, **When** the daily section renders, **Then** one daily table per compared year appears below the summary table, in chronological order, each labeled with its year.
2. **Given** a daily table for a compared year, **When** its cells render, **Then** the values, per-row visibility, precision, zero-handling, and threshold coloring match the monthly view for that same month and year.
3. **Given** a compared year without recorded data for the month, **When** the daily section renders, **Then** that year shows an empty state instead of fabricated values.
...
- **FR-003**: The comparison view MUST lead with a summary table whose rows are the configured entities/expressions (as in the monthly and yearly views) and whose columns are the compared years; each cell presents the selected month's summary values — min/avg/max for measurement rows (honoring the row's configured min/avg/max visibility) and the monthly total for cumulative rows — together with that value's two differences (FR-005, FR-006).
...
- **FR-009**: Below the summary table, the comparison view MUST render the selected month's daily values for each compared year as per-year tables presented like the monthly view, in chronological order, each labeled with its year.
...
- The daily detail tables are stacked one per year in chronological order, matching the yearly view's year ordering; a side-by-side day alignment across years is not part of this feature.
