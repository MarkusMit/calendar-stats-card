...
- **FR-004**: The monthly **total** column MUST remain unaffected by `show_zero` (its derivation is governed by `specs/011-ha-monthly-totals/spec.md` FR-002: total = HA monthly `sum[month] − sum[prev_month]` for cumulative entity rows, arithmetic daily sum for expression rows; in either case `show_zero: false` does NOT remove zero days from the total).
